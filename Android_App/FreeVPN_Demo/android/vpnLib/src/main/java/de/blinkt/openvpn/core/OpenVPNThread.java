...
public class OpenVPNThread implements Runnable {
    ...
    private String genLibraryPath(String[] argv, ProcessBuilder pb) {
        // argv[0] is the full path to the openvpn executable (libovpnexec.so)
        // inside the app's native library dir. That same directory also holds
        // the libopenvpn.so it dynamically links against, so the library path
        // must be the *directory*, not the executable's file path.
        String applibpath = new File(argv[0]).getParent();
        ...
        if (mNativeDir != null && !applibpath.equals(mNativeDir)) {
